...
try {
    ...
    Assert-True ($commentText -match 'io\.realdiff\.demo\.pricing\.CheckoutTotals\.compute\(D\)D' `
        ...
        'Java default comment lost the known-true finding'
    ...
    Assert-True ($strictCommentText -match 'io\.realdiff\.demo\.pricing\.CheckoutTotals\.compute\(D\)D' `
        ...
        -and $strictCommentText -match 'Double:60\.0' -and $strictCommentText -match 'Double:85\.0' `
        -and $strictCommentText -match '2 of the 3 tests that executed this did not assert on the change' `
        -and $strictCommentText -notmatch 'SampleApp|Commerce\.Pricing|Infrastructure\.Collections') `
        'Strict Java comment lost required evidence or contains .NET demo contamination'
