...
$work = Join-Path ([IO.Path]::GetTempPath()) "realdiff-anthropic-$runId"
$prTree = Join-Path ([IO.Path]::GetTempPath()) "realdiff-anthropic-pr-$runId"
...
try {
    ...
    & dotnet run --project (Join-Path $PSScriptRoot 'AnthropicProof/RealDiff.AnthropicProof.csproj') `
        -c Release -- $findings
