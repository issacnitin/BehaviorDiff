...
class Scope:
    ...
    def from_environment(cls) -> "Scope":
        root_value = os.environ.get("BEHAVIORDIFF_REPOSITORY_ROOT")
        if not root_value:
            raise RuntimeError("BEHAVIORDIFF_REPOSITORY_ROOT is required for Python tracing")
        includes = _read_list(os.environ.get("BEHAVIORDIFF_INCLUDE_NAMESPACES", ""))
        excludes = _read_list(os.environ.get("BEHAVIORDIFF_EXCLUDE_NAMESPACES", ""))
...
class Monitor:
    def __init__(self, scope: Scope, observer: Callable[[str, CodeType, object | None], None] | None = None):
        self.scope = scope
    ...
    def _observe(self, event: str, code: CodeType, value: object | None) -> None:
        if self.observer is not None:
            self.observer(event, code, value)


_monitor: Monitor | None = None


def install(observer: Callable[[str, CodeType, object | None], None] | None = None) -> Monitor:
    global _monitor
    if sys.version_info < MINIMUM_VERSION or not hasattr(sys, "monitoring"):
        raise RuntimeError("BehaviorDiff Python tracing requires Python 3.12+ with sys.monitoring")
    if _monitor is not None:
        return _monitor
    if sys.monitoring.get_tool(TOOL_ID) is not None:
        raise RuntimeError(f"sys.monitoring tool id {TOOL_ID} is already in use")

    monitor = Monitor(Scope.from_environment(), observer)
    events = sys.monitoring.events
    sys.monitoring.use_tool_id(TOOL_ID, "behaviordiff")
