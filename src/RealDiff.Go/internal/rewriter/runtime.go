...
import (
	...
	"github.com/realdiff/realdiff-go/internal/runtime/canonical"
)

const runtimeSource = `package realdiffrt
...
var sensitiveNames = append(defaultSensitiveNames, configuredList("REALDIFF_REDACT_NAMES")...)
var digestOnlyTypes = configuredList("REALDIFF_REDACT_TYPES")
var digestOnlyPaths = configuredList("REALDIFF_REDACT_PATHS")

func init() {
	configured := os.Getenv("REALDIFF_TRACE")
	if configured == "" {
		return
	}
	...
	if err := os.MkdirAll(filepath.Dir(state.tracePath), 0755); err != nil {
		panic(fmt.Errorf("realdiff: create trace directory: %w", err))
	}
	trace, err := os.Create(state.tracePath)
	if err != nil {
		panic(fmt.Errorf("realdiff: create trace: %w", err))
	}
	state.trace = trace
	state.Lock()
	defer state.Unlock()
...
func Exit(frame *Frame, results []any, recovered any) {
	...
	frame.exit.Do(func() {
		...
		if err != nil {
			panic(fmt.Errorf("realdiff: encode event: %w", err))
		}
		state.Lock()
		defer state.Unlock()
		if state.trace == nil {
		...
		if _, err := state.trace.Write(append(line, '\n')); err != nil {
			panic(fmt.Errorf("realdiff: write event: %w", err))
		}
		state.written++
...
func decorate(configured, suffix string) string {
	...
	if err != nil {
		panic(fmt.Errorf("realdiff: resolve trace path: %w", err))
	}
...
func writeManifestLocked() {
	...
	for _, record := range records {
		...
		if err != nil { panic(fmt.Errorf("realdiff: encode manifest: %w", err)) }
		output.Write(line)
		output.WriteByte('\n')
	}
	temporary := state.manifestPath + ".tmp"
	if err := os.WriteFile(temporary, []byte(output.String()), 0644); err != nil {
		panic(fmt.Errorf("realdiff: write manifest: %w", err))
	}
	if err := os.Rename(temporary, state.manifestPath); err != nil {
		...
		if retryErr := os.Rename(temporary, state.manifestPath); retryErr != nil {
			panic(fmt.Errorf("realdiff: replace manifest: %w", retryErr))
		}
	}
}
`
...
func writeRuntime(out string) error {
	directory := filepath.Join(out, "internal", "realdiffrt")
	...
	generated := strings.NewReplacer(
		"package canonical", "package realdiffrt",
