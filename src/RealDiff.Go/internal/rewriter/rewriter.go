...
func Rewrite(options Options) (report Report, err error) {
	...
	runtimeImport := modulePath + "/internal/realdiffrt"
	if _, err := os.Stat(filepath.Join(source, "internal", "realdiffrt")); err == nil {
		return Report{}, fmt.Errorf("source already contains internal/realdiffrt")
