...
function createScope(options = {}, environment = process.env) {
  const includes = readList(options.include ?? environment.BEHAVIORDIFF_NAMESPACES);
  const excludes = readList(options.exclude ?? environment.BEHAVIORDIFF_EXCLUDE_NAMESPACES);
  if (includes.length === 0) {
    throw new Error('BehaviorDiff Node requires an include scope');
  }
