...
function runChild(t, kind) {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), `behaviordiff-${kind}-`));
  t.after(() => fs.rmSync(directory, { recursive: true, force: true }));
  fs.mkdirSync(path.join(directory, 'app'));
  const tracePath = path.join(directory, 'run.ndjson');
  const environment = {
    ...process.env,
    BEHAVIORDIFF_TRACE: tracePath,
    BEHAVIORDIFF_NAMESPACES: 'app'
  };

  let argumentsList;
  if (kind === 'cjs') {
    environment.BEHAVIORDIFF_EXCLUDE_NAMESPACES = 'app/excluded.js';
    ...
    fs.writeFileSync(path.join(directory, 'runner.cjs'), `
      ...
      globalThis[Symbol.for('behaviordiff.runtime')].withTestRoot('cjs#1', () => subject.work(4));
    `);
    ...
    fs.writeFileSync(path.join(directory, 'runner.mjs'), `
      ...
      await globalThis[Symbol.for('behaviordiff.runtime')].withTestRoot('esm#1', () => work(4));
    `);
