...
const RUNTIME_SYMBOL = Symbol.for('realdiff.runtime');
const SCHEMA = 'realdiff.trace/1';
...
function createRuntime(options = {}) {
  const configuredPath = options.tracePath ?? process.env.REALDIFF_TRACE;
  const processId = options.processId ?? process.pid;
  const storage = new AsyncLocalStorage();
  const redaction = {
    sensitiveNames: configuredList('REALDIFF_REDACT_NAMES', DEFAULT_REDACT_NAMES),
    digestOnlyTypes: configuredList('REALDIFF_REDACT_TYPES'),
    digestOnlyPaths: configuredList('REALDIFF_REDACT_PATHS').map(normalizedPath),
  };
