...
fn route(args: &[OsString]) -> Result<Route, String> {
    ...
    if first == "detect" || first == "detect-language" {
        return (args.len() == 2)
            ...
            .ok_or_else(|| "usage: behaviordiff detect <repo>".to_owned());
    }
    ...
    while index < args.len() {
        ...
        if argument == "--engine" || argument.starts_with("--engine=") {
            return Err("--engine was removed; BehaviorDiff uses the Rust engine.".to_owned());
        }
...
fn spawn_managed(args: &[OsString], handoff: Option<&Path>) -> i32 {
    ...
    command.args(args);
    if let Some(path) = handoff {
        command.env("BEHAVIORDIFF_LAUNCHER_CONFIG", path);
    }
    match command.status() {
        Ok(status) => status.code().unwrap_or(FAILED),
        Err(error) => {
            eprintln!("BehaviorDiff managed component could not start: {error}");
            FAILED
        }
    }
}
...
fn managed_path() -> Result<PathBuf, String> {
    if let Some(configured) = std::env::var_os("BEHAVIORDIFF_MANAGED_CLI") {
        let path = PathBuf::from(configured);
        return path.is_file().then_some(path).ok_or_else(|| {
            "BEHAVIORDIFF_MANAGED_CLI does not name an existing executable.".to_owned()
        });
    }
    let directory = std::env::current_exe()
        .map_err(|error| format!("Could not locate the BehaviorDiff launcher: {error}"))?
        .parent()
        .ok_or_else(|| "BehaviorDiff launcher has no parent directory.".to_owned())?
        .to_owned();
    let name = if cfg!(windows) {
        "behaviordiff-managed.exe"
    } else {
        "behaviordiff-managed"
    };
    let path = directory.join(name);
    path.is_file().then_some(path.clone()).ok_or_else(|| {
        format!(
            "BehaviorDiff managed component was not found: {}",
            path.display()
        )
    })
}
...
fn write_handoff(config: &LoadedConfig) -> Result<PathBuf, String> {
    ...
    let json = serde_json::to_vec(&envelope)
        .map_err(|error| format!("Could not serialize BehaviorDiff config: {error}"))?;
    let nonce = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .map_err(|error| format!("Could not timestamp BehaviorDiff config handoff: {error}"))?
        .as_nanos();
    for attempt in 0..10 {
        let path = std::env::temp_dir().join(format!(
            "behaviordiff-launcher-config-{}-{nonce}-{attempt}.json",
            std::process::id()
        ));
        ...
        match options.open(&path) {
            Ok(mut file) => {
                file.write_all(&json).map_err(|error| {
                    format!("Could not write BehaviorDiff config handoff: {error}")
                })?;
                return Ok(path);
            }
            ...
            Err(error) => {
                return Err(format!(
                    "Could not create BehaviorDiff config handoff: {error}"
                ));
            }
        }
    }
    Err("Could not allocate a unique BehaviorDiff config handoff file.".to_owned())
}
...
fn load_config(repository: &Path) -> Result<LoadedConfig, String> {
    ...
    let path = root.join(".behaviordiff").join("config.yml");
    let exists = path.is_file();
    let value = if exists {
        let content = fs::read_to_string(&path)
            .map_err(|error| format!("Could not read BehaviorDiff config: {error}"))?;
        if content.trim().is_empty() {
            return Err(format!("BehaviorDiff config is empty: {}", path.display()));
        }
        serde_yaml::from_str(&content)
            .map_err(|error| format!("BehaviorDiff config YAML is malformed: {error}"))?
    } else {
...
fn validate_config(config: &RepositoryConfig, path: &Path) -> Result<(), String> {
    ...
    for (name, values) in [
        ...
        if values.iter().any(|value| value.trim().is_empty()) {
            return Err(format!(
                "BehaviorDiff config {name} contains an empty value."
            ));
        }
    }
    Ok(())
...
fn resolve_workdir(config: &LoadedConfig) -> Result<PathBuf, String> {
    ...
    let path = config
        ...
        .join(relative)
        .canonicalize()
        .map_err(|_| format!("BehaviorDiff config workdir does not exist: {relative}"))?;
    if !path.starts_with(&config.root) {
        return Err(format!(
            "BehaviorDiff config workdir escapes the repository root: {relative}"
        ));
    }
    path.is_dir()
        .then_some(path)
        .ok_or_else(|| format!("BehaviorDiff config workdir does not exist: {relative}"))
}
...
fn render(detection: &Detection) -> String {
    ...
    output.push_str(if detection.configured {
        ".behaviordiff/config.yml + detection"
    } else {
        "auto-detection"
    });
    output
...
mod tests {
    ...
    fn config_overrides_detection() {
        let directory = tempdir().unwrap();
        fs::create_dir_all(directory.path().join("services/web/.behaviordiff")).unwrap();
        fs::create_dir_all(directory.path().join(".behaviordiff")).unwrap();
        fs::write(directory.path().join("services/web/package.json"), "").unwrap();
        fs::write(
            directory.path().join(".behaviordiff/config.yml"),
        ...
        assert!(output.contains("source: .behaviordiff/config.yml + detection"));
    }
    ...
    fn handoff_uses_managed_property_names() {
        ...
        let envelope = ConfigEnvelope {
            config_path: Path::new(".behaviordiff/config.yml"),
            config: &config,
        };
