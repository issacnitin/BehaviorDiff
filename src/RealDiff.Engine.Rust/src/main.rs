...
fn print_usage() {
    println!("RealDiff.Engine.Rust");
    println!(
        "usage: realdiff-engine diff --base1 <dir> --base2 <dir> --pr <dir> --out <file.json>"
    );
    println!(
        "       realdiff-engine stream-probe --base1 <dir> --base2 <dir> --base3 <dir> --pr <dir> --out <report.json>"
    );
    println!(
        "       realdiff-engine stream-diff --base1 <dir> --base2 <dir> --base3 <dir> --pr <dir> --out <set.json>"
    );
    println!("       realdiff-engine frontier --in <set.json> --changed-files <paths.txt> --out <report.json>");
    println!("       realdiff-engine findings --divergences <set.json> --frontier <report.json> --out <findings.json> --exit-code <code> --base-sha <sha> --pr-sha <sha> --merge-base <sha>");
    println!("       realdiff-engine findings-invalid --status <refused|failed> --exit-code <code> --reason <text> --out <findings.json>");
    println!(
        "       realdiff-engine baseline --findings <findings.json> --baseline <baseline.yml>"
    );
    println!("       realdiff-engine baseline-validate --baseline <baseline.yml>");
    println!("       realdiff-engine read <trace.ndjson>");
    println!("       realdiff-engine normalize <trace.ndjson> -o <output.ndjson> [--force]");
}
