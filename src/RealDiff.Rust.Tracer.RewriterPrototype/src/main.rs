...
const CACHE_VERSION: &str = "realdiff-rust-rewriter-prototype/1";
...
fn emit(method: &str, file: &str, line: u32, outcome: &str) {
    let Ok(path) = std::env::var("REALDIFF_RUST_TRACE") else { return; };
...
impl Instrumenter {
    fn instrument(&self, name: &syn::Ident, is_async: bool, block: &mut Block) {
        ...
        *block = if is_async {
            parse_quote!({
                let __realdiff_guard = crate::__realdiff_runtime::enter(#method, file!(), line!());
                let __realdiff_result = (async move #original).await;
                __realdiff_guard.complete();
                __realdiff_result
            })
        } else {
            parse_quote!({
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if arguments.len() != 3 {
        return Err("usage: realdiff-rust-rewriter-prototype <source> <cache-root>".into());
    }
    ...
    for path in rust_files {
        ...
        if path == source_root.join("main.rs") || path == source_root.join("lib.rs") {
            syntax.items.insert(
                0,
                parse_quote!(
                    mod __realdiff_runtime;
                ),
            );
        }
        fs::write(path, prettyplease::unparse(&syntax))?;
    }
    fs::write(source_root.join("__realdiff_runtime.rs"), RUNTIME)?;
    Ok(())
}
