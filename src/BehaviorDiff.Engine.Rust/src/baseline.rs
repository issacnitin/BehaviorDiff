...
impl Default for BaselineDocument {
    fn default() -> Self {
        Self {
            schema: "behaviordiff.baseline/2".to_owned(),
...
fn apply(options: &BaselineOptions, today: Date) -> Result<i32, String> {
    ...
    if text(&root, "schema") != "behaviordiff.findings/1" || text(&root, "status") != "analyzed" {
        return Err(
            "A baseline can only be applied to analyzed behaviordiff.findings/1 artifacts."
                .to_owned(),
        );
    }
...
fn validate(baseline: &BaselineDocument) -> Result<(), String> {
    if baseline.schema != "behaviordiff.baseline/2" {
        return Err(format!(
...
fn display_path(path: &str) -> Result<String, String> {
    ...
    if let Some(marker) = normalized.rfind("/.behaviordiff/") {
