...
fn emit(guard: &Guard, outcome: &str, result: Option<&CapturedValue>) {
    ...
    let _ = writeln!(
        stream,
        "{{\"schema\":\"realdiff.trace/1\",\"testId\":\"{test_id}\",\"methodFullName\":\"{method}\",\"filePath\":\"{file}\",\"filePathResolution\":\"debugInfo\",\"line\":{line},\"callDepth\":{call_depth}{parent},\"callId\":{call_id},\"ordinal\":{ordinal},\"threadId\":{thread_id},\"isHarness\":{is_test_root},\"outcome\":\"{outcome}\"{args}{result}{exception}}}"
    );
}
...
fn open_trace() -> Option<File> {
    let path = std::env::var_os("REALDIFF_RUST_EXIT_TRACE")?;
