...
const CACHE_VERSION: &str = "realdiff.rust-rewrite-cache/6";
const ORIGIN_MANIFEST: &str = ".realdiff-rust-origin.json";
...
pub fn finalize(origin: &Path, trace: &Path, output: &Path) -> Result<FinalizeReport, String> {
    ...
    lines.push(serde_json::json!({
        "kind": "run", "schema": "realdiff.trace/1", "language": "rust"
    }));
    lines.push(serde_json::json!({
...
fn struct_reader(item: &ItemStruct, local_types: &HashSet<String>) -> Result<ItemImpl, String> {
    ...
    syn::parse2(quote::quote! {
        impl #impl_generics ::realdiff_rust_runtime::Canonicalize for #ident #type_generics #where_clause {
            fn write_canonical(
                &self,
                context: &mut ::realdiff_rust_runtime::CanonicalContext,
                output: &mut String,
            ) {
...
fn field_write(
    ...
    if exact_type(ty, local_types, generic_names) {
        quote::quote! {
            ::realdiff_rust_runtime::write_field(context, output, #label, #value);
        }
    } else {
...
impl ExitHookInstrumenter {
    ...
    fn instrument(&mut self, signature: &syn::Signature, block: &mut Block, is_test_root: bool) {
        ...
        *block = if signature.asyncness.is_some() {
            parse_quote!({
                let __realdiff_args = ::realdiff_rust_runtime::capture_arguments(vec![#(#args),*]);
                let __realdiff_guard = ::realdiff_rust_runtime::enter(#method_expression, #file, #line, __realdiff_args, #is_test_root);
                let (__realdiff_guard, __realdiff_result) =
                    ::realdiff_rust_runtime::trace_future(
                        __realdiff_guard,
                        async move #original,
                    ).await;
                let __realdiff_return = #result;
                __realdiff_guard.complete(__realdiff_return);
                __realdiff_result
            })
        } else {
            parse_quote!({
                let __realdiff_args = ::realdiff_rust_runtime::capture_arguments(vec![#(#args),*]);
                let __realdiff_guard = ::realdiff_rust_runtime::enter(#method_expression, #file, #line, __realdiff_args, #is_test_root);
                let __realdiff_result = (|| #original)();
                let __realdiff_return = #result;
                __realdiff_guard.complete(__realdiff_return);
                __realdiff_result
            })
...
fn argument_capture(
    ...
    signature
        ...
        .map(|argument| match argument {
            FnArg::Receiver(_) => {
                let detail = "receiver";
                quote::quote!(("self", ::realdiff_rust_runtime::capture_skipped(#detail)))
            }
            FnArg::Typed(argument) => {
                let Pat::Ident(pattern) = argument.pat.as_ref() else {
                    return quote::quote!((
                        "pattern",
                        ::realdiff_rust_runtime::capture_skipped("pattern")
                    ));
                };
                let ident = &pattern.ident;
                let name = ident.to_string();
                if exact_type(&argument.ty, local_types, generic_names) {
                    quote::quote!((#name, ::realdiff_rust_runtime::capture(&#ident)))
                } else {
                    let detail = skipped_type_detail(&argument.ty, generic_names);
                    quote::quote!((#name, ::realdiff_rust_runtime::capture_skipped(#detail)))
                }
            }
        })
        .collect()
...
fn return_capture(
    ...
    match output {
        ...
        ReturnType::Type(_, ty) => {
            let detail = skipped_type_detail(ty, generic_names);
            quote::quote!(Some(::realdiff_rust_runtime::capture_skipped(#detail)))
        }
    }
}

fn write_runtime(staging: &Path) -> Result<(), String> {
    let runtime = staging.join(".realdiff/runtime");
...
fn inject_runtime_dependencies(staging: &Path, files: &[PathBuf]) -> Result<(), String> {
        ...
        .filter(|path| path.file_name().is_some_and(|name| name == "Cargo.toml"))
    {
        ...
        document["dependencies"]["realdiff-rust-runtime"] =
...
mod tests {
    ...
    fn cache_inside_source_is_rejected() {
        let root = std::env::temp_dir().join(format!(
            "realdiff-rust-rewrite-nested-test-{}-{}",
            std::process::id(),
            unique_suffix()
        ));
