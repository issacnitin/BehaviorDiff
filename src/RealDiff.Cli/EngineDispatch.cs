...
namespace RealDiff.Cli
{
    internal static class EngineDispatch
    {
        ...
        {
            string? configured = Environment.GetEnvironmentVariable("REALDIFF_RUST_ENGINE");
            if (!string.IsNullOrWhiteSpace(configured))
            {
                string fullPath = Path.GetFullPath(configured);
                if (!File.Exists(fullPath))
                {
                    throw new CliException("REALDIFF_RUST_ENGINE does not name an existing executable: " + fullPath);
                }
            ...
            {
                string source = Path.Combine(root, "src", "RealDiff.Engine.Rust", "target", "release", fileName);
            ...
            throw new CliException(
                "RealDiff Rust engine was not found. Set REALDIFF_RUST_ENGINE, install "
                + packaged + ", or run cargo build --release for src/RealDiff.Engine.Rust.");
        }
