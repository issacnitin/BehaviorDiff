...
namespace RealDiff.Cli
{
    ...
    internal static class RepoScan
    {
        ...
        private static readonly Regex Excluded =
            new(@"<RealDiffExclude>\s*true\s*</RealDiffExclude>", RegexOptions.IgnoreCase | RegexOptions.Compiled);
        ...
        internal static RepoScanResult Scan(string root)
        {
            ...
            foreach (string project in Directory.EnumerateFiles(root, "*.csproj", SearchOption.AllDirectories))
            {
                ...
                if (!name.StartsWith("RealDiff.", StringComparison.Ordinal) && name != "RealDiff")
                {
                ...
                foreach (Match match in ProjectNamespace.Matches(text))
                {
                    ...
                    if (!string.Equals(prefix, "RealDiff", StringComparison.Ordinal))
                    {
    ...
    internal static class InjectionKit
    {
        ...
        internal static string Build(string workDirectory)
        {
            ...
            File.WriteAllText(Path.Combine(kit, "RealDiffBootstrap.cs"), Bootstrap, new UTF8Encoding(false));
            File.WriteAllText(Path.Combine(kit, "RealDiff.Inject.targets"), Targets, new UTF8Encoding(false));
        ...
        private const string Bootstrap = """
            [assembly: RealDiff.Tracer.TraceTest]
            """;

        private const string Targets = """
            <Project>
              <PropertyGroup>
                <RealDiffIsTarget Condition="'$(RealDiffTestProjects)' != '' And $(RealDiffTestProjects.ToLowerInvariant().Contains($(MSBuildProjectFullPath.ToLowerInvariant())))">true</RealDiffIsTarget>
                ...
                <RealDiffInject Condition="'$(RealDiffIsTarget)' == 'true' And '$(TargetFramework)' == '$(RealDiffTraceTfm)'">true</RealDiffInject>
                 ...
                 <NoWarn Condition="'$(RealDiffInject)' == 'true'">$(NoWarn);CS8002</NoWarn>
              </PropertyGroup>

              <ItemGroup Condition="'$(RealDiffInject)' == 'true'">
                <Compile Include="$(RealDiffKitDir)RealDiffBootstrap.cs" Link="RealDiffBootstrap.cs" />

                <Reference Include="RealDiff.Contracts">
                  <HintPath>$(RealDiffKitDir)RealDiff.Contracts.dll</HintPath>
                  <Private>true</Private>
                </Reference>
                <Reference Include="RealDiff.Tracer">
                  <HintPath>$(RealDiffKitDir)RealDiff.Tracer.dll</HintPath>
                  <Private>true</Private>
                </Reference>
                <Reference Include="RealDiffAdapter">
                  <HintPath>$(RealDiffAdapterPath)</HintPath>
                  <Private>true</Private>
                </Reference>

                <None Include="$(RealDiffKitDir)runtime\*.dll">
