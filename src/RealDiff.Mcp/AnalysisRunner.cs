...
namespace RealDiff.Mcp;

/// <summary>
/// Invokes the existing RealDiff CLI. It does not reimplement any part of the analysis; it shells
/// out, records the exit code, and copies the CLI's own artifacts into the run directory.
/// </summary>
public static class AnalysisRunner
{
    /// <summary>Absolute path to the built RealDiff CLI. Overridable for tests and packaging.</summary>
    public static string CliPath { get; set; } =
        Environment.GetEnvironmentVariable("REALDIFF_CLI") ?? "realdiff";
    ...
    private static void Run(RunRecord record)
    {
        ...
        try
        {
            ...
            if (process is null)
            {
                Fail(record, "could not start the RealDiff CLI at '" + CliPath + "'");
                return;
            }
